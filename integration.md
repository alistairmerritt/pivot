...
> **Bank assignments live in these text inputs.** The `bank_N_entity` text entities are the single source of truth for what each bank controls. The configure dialog (**Settings → Devices & Services → Pivot → Configure**) is a convenient editor for them – with an entity picker and the timer-bank selector – but editing the text inputs directly is fully supported and persists across restarts. That includes changing them **from automations**: for example, an automation can point a bank at different lights depending on the time of day. One caveat: the field validates the entity ID *format* but not that the entity exists, so a typo leaves the bank silently doing nothing – the configure dialog's picker avoids this.
