...
| `number.{suffix}_secondary_control_value` | Secondary scroll value (0–100%) |
...
| `triple_press` | Announces current mode and bank via TTS (if announcements configured) |
...
Pivot writes dedicated files it fully owns — it never reads or rewrites your existing `scripts.yaml` or `automations.yaml`.
...
The first time Pivot sets up a device it appends a single `!include` line to each of your files. That is the only time it touches them. If you remove the integration or switch modes, Pivot removes its own files and the `!include` lines.

Before appending anything, Pivot backs up your files:
- `scripts.yaml.pivot_backup`
- `automations.yaml.pivot_backup`

Pivot also validates generated YAML before writing — if invalid, it aborts and logs an error rather than writing a broken file.
