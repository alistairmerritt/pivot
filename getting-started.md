...
Wireless updates can also be authenticated with your <code>api_encryption_key</code> instead of the password, so keep that key just as safe. The <a href="/pivot/firmware/#ota_password">Firmware</a> page covers both in full.<br><br>
...
> **Skipped the entity assignment screen?** You can still assign entities at any time by either setting the `text.{device_suffix}_bank_N_entity` entities directly from your Pivot device page or in your device configuration settings (⚙️).
