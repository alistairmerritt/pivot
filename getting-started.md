...
<blockquote><strong>OTA password is required for security purposes.</strong> Once your device runs Pivot firmware, all future updates can happen wirelessly (though USB is still acceptable too). Without a password, anyone who can reach the device on your network could install their own firmware onto it. Your <code>api_encryption_key</code> also authenticates wireless updates, so keep it just as safe. <br><br>
