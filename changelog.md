...
- **New (Timer blueprint):** Paused timers auto-cancel after 30 minutes. If the timer is left in `paused` state for more than 30 minutes, the gauge resets to 0 and `timer_state` returns to `idle` automatically on the next 5-second gauge sync.
